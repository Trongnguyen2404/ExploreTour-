...
// Page<Location> findAll(Pageable pageable); // Có sẵn
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Location l SET l.createdByAdmin = NULL WHERE l.createdByAdmin = :adminUser")
    void nullifyCreatedByAdmin(@Param("adminUser") User adminUser);

    @Modifying
    @Transactional
    @Query("UPDATE Location l SET l.updatedByAdmin = NULL WHERE l.updatedByAdmin = :adminUser")
